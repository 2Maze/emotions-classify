...
def load_data(bath_size=10, validation_bath_size=None, num_workers=1, test_size=0.15, dataset_class=EmotionDataset):
    validation_bath_size = validation_bath_size or bath_size

    dataset = dataset_class(annotation=join(ROOT_DIR, 'data', 'dataset', 'annotations.json'), padding_sec=PADDING_SEC)
    dataset = dataset
